...
const [ArtistUploadingProcess, setArtistUploadingProcess] = useState(0);
    ...
    if (!ArtistImageCover || !artistName ||!twitter || !instagram) {
      ...
      saveNewArtist(data).then((res) => {
        getAllArtist().then((artists) => {
          dispath({ type: actionType.SET_ALL_ARTISTS, artists: data.artist });
        });
      });
      dispath({
        type:actionType.SET_ALERT_TYPE,
...
<button type="button" className="absolute bottom-3 right-3 p-3 rounded-full text-xl bg-red-500 cursor-pointer outline-none border-none hover:shadow-md duration-200 transition-all ease-in-out" onClick={()=>deleteFileObject(songImageCover,true)}>
...
<button type="button" className="absolute bottom-3 right-3 p-3 rounded-full text-xl bg-red-500 cursor-pointer outline-none border-none hover:shadow-md duration-200 transition-all ease-in-out" onClick={()=>deleteFileObject(AudioImageCover,false)}>
  <MdDelete className="text-white" />
</button>
...
<div className="flex items-center justify-end w-60 cursor-pointer p-4">
...
<div className="bg-card backdrop-blur-md w-full h-300 rounded-md border-2 border-dotted border-gray-300 cursor-pointer">
{IsArtistUploading && <FileLoader progress={ArtistUploadingProcess}/>}
{!IsArtistUploading && (
<>
  {!ArtistImageCover ?(
  <FileUpLoader updateState={setArtistImageCover} setProgress={setArtistUploadingProcess} isLoading={setIsArtistUploading} isImage={true} />)
:
 ( <div className="relative w-full h-full overflow-hidden rounded-md">
<img src={ArtistImageCover} className="w-full h-full object-cover" alt="" />
<button type="button" className="absolute bottom-3 right-3 p-3 rounded-full text-xl bg-red-500 cursor-pointer outline-none border-none hover:shadow-md duration-200 transition-all ease-in-out" onClick={()=>deleteFileObject(ArtistImageCover,true)}>
  <MdDelete className="text-white" />
</button>
...
<button type="button" className="absolute bottom-3 right-3 p-3 rounded-full text-xl bg-red-500 cursor-pointer outline-none border-none hover:shadow-md duration-200 transition-all ease-in-out" onClick={()=>deleteFileObject(AlbumImageCover,true)}>
  <MdDelete className="text-white" />
</button>
